...
pub enum SparseError {
    ...
    Empty,
    /// Right-hand side or numerical entry count does not match the symbolic system.
    DimensionMismatch,
    /// A sparse entry references a row or column outside the square matrix.
    IndexOutOfBounds,
    /// A numerical matrix entry or right-hand-side value is NaN or infinite.
    NonFinite,
    /// Numerical entries do not match the cached sparsity pattern.
    PatternMismatch,
...
impl SparseLinearSystem {
    /// Builds and caches a square sparsity pattern and symbolic LU analysis.
    ///
    /// # Errors
    ...
    pub fn new(dimension: usize, entries: &[SparseEntry]) -> Result<Self, SparseError> {
        ...
        if entries
            .iter()
            .any(|entry| entry.row >= dimension || entry.col >= dimension)
        {
            return Err(SparseError::IndexOutOfBounds);
        }
        if entries.iter().any(|entry| !entry.value.is_finite()) {
    ...
    ///
    /// # Errors
    /// Returns [`SparseError::PatternMismatch`] if coordinates differ from the cached pattern,
    /// [`SparseError::DimensionMismatch`] for incompatible entry/RHS lengths, and typed numerical
    /// errors for non-finite or unsolved systems.
    pub fn factor_and_solve(
        &self,
        entries: &[SparseEntry],
        rhs: &[f64],
    ) -> Result<SparseSolution, SparseError> {
        if entries.len() != self.pattern.len() || rhs.len() != self.dimension {
