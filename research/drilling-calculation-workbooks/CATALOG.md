...
| --- | ---: | --- |
| `.xlsx` | 49 | Yes |
| `.xls` | 39 | No, filename and path evidence only |
| `.xlsm` | 10 | Yes, macros not executed |
| `.xlsb` | 2 | No, filename and path evidence only |

## Temperature-model leads

| Workbook | Static evidence | Research relevance |
| --- | --- | --- |
| `HyperLine TM Smart Toolbox v5.xlsm` | 28,879 formulas; dedicated `Thermal` and `Hydraulics` sheets | Coupled downhole tool, thermal, and hydraulic behavior |
| `SPT - Geothermal.xlsx` | 357 formulas | Formation/geothermal temperature calculations |
| `Thermal Model U calc data.xls` | Legacy binary workbook; metadata-only classification | Heat-transfer coefficient and thermal-model source candidate |
